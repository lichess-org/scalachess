...
class FenTest extends ChessTest:
  ...
  test("write: enpassant"):
    val sit = Replay.mainline("d4 e5 d5 c5").flatMap(_.valid).get
    assertEquals(
      Fen.write(sit.state),
      FullFen("rnbqkbnr/pp1p1ppp/8/2pPp3/8/8/PPP1PPPP/RNBQKBNR w KQkq c6 0 3")
    )

  test("write: enpassant 2"):
    val sit = Replay.mainline("e4 c5 e5 f5").flatMap(_.valid).get
    assertEquals(
      Fen.write(sit.state),
      FullFen("rnbqkbnr/pp1pp1pp/8/2p1Pp2/8/8/PPPP1PPP/RNBQKBNR w KQkq f6 0 3")
    )
  ...
  test("write: enpassant in game"):
    ...
    val sit = Replay.mainline(pgn).flatMap(_.valid).get
    assertEquals(Fen.write(sit.state), FullFen("5rk1/ppp3pp/2np4/2q5/3Pp1n1/NPP5/PB2Q1PP/R5KR b - d3 0 17"))

  test("write: not enpassant in game"):
    val pgn =
    ...
    assertEquals(
      Fen.write(sit.state),
      FullFen("3r1r2/1p5k/pb1q2p1/3p2Pp/4pP2/2B1P1P1/PP1Q2K1/R6R w - - 0 28")
    )

  test("read: enpassant"):
    assertEquals(
      Fen
        .read(Fen.Full("1brr2k1/1pq3p1/p6p/P1p1Pp2/2P5/3PRB2/2Q2PPP/R5K1 w - f6 0 28"))
        .get
        .enPassantSquare,
      Some(Square.F6)
    )

  test("read: enpassant 2"):
    assertEquals(
      Fen
        .read(Fen.Full("rnbqkbnr/pp1pp1pp/8/2p1Pp2/8/8/PPPP1PPP/RNBQKBNR w KQkq f6 0 3"))
        .get
        .enPassantSquare,
      Some(Square.F6)
    )

  test("castling rights with 2 rooks on the same file"):
    val fen = Fen.Full("2bqkb1r/1pp1ppp1/7r/pN2p2p/3PP3/P3P3/1PP1B1PP/R2Q1RK1 w k -")
  ...
  test("catsling rights with 2 rooks on the king side"):
    ...
    assertEquals(board.history.castlingRights, CastlingRights(Square.H1.bl))
  ...
  test("castling rights with 2 rooks on the same side"):
    ...
    val f3 = Fen.Full("4k3/8/8/8/8/8/8/RR2K3 w B -")
    val f4 = Fen.Full("4k3/8/8/8/8/8/8/RR2K3 w Q -")
    val f5 = Fen.Full("rr2k3/8/8/8/8/8/8/4K3 w b -")
    val f6 = Fen.Full("rr2k3/8/8/8/8/8/8/4K3 w q -")
    val f7 = Fen.Full("4k1rr/8/8/8/8/8/8/4K3 w g -")
