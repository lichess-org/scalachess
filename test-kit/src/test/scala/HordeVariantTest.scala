...
class HordeVariantTest extends ChessTest:
  ...
  test("Must not auto-draw if not all black King moves leads to stalemate"):
    ...
    assertNot(Horde.isInsufficientMaterial(game.board))
    assertNot(game.board.opponentHasInsufficientMaterial)
  ...
  test("Pawn on first rank should able to move two squares"):
    ...
    val game     = fenToGame(position, Horde)
    assert(game.board.legalMoves.exists(m => m.orig == Square.D1 && m.dest == Square.D3))
  ...
  test("Cannot en passant a pawn from the first rank"):
    ...
    assertNot(game._1.board.legalMoves.exists(m => m.orig == Square.B3 && m.dest == Square.C2))
  ...
  test("the h8 rooks move"):
    ...
    assertEquals(game._1.board.history.unmovedRooks, UnmovedRooks(Set(Square.A8)))
    assertEquals(game._1.board.history.castles, Castles(false, false, false, true))
