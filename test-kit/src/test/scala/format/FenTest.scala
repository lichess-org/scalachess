...
class FenTest extends ChessTest:
  ...
  test("write: enpassant"):
    val sit = Reader.mainline("d4 e5 d5 c5").flatMap(_.valid).get
    assertEquals(
      Fen.write(sit.state),
  ...
  test("write: enpassant 2"):
    val sit = Reader.mainline("e4 c5 e5 f5").flatMap(_.valid).get
    assertEquals(
      Fen.write(sit.state),
  ...
  test("write: enpassant in game"):
    ...
    val sit = Reader.mainline(pgn).flatMap(_.valid).get
