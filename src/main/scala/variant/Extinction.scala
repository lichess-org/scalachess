...
case object Extinction extends Variant(
  ...
  name = "Extinction",
  shortName = "extinct",
  title = "Capture a piece to extinction to win the game.",
  ...
  // the king is an ordinary piece in extinction chess, so we don't filter
  // promotion of your last pawn loses the game, so it is not allowed
  // that logic will go in here for now (although it's pretty unrelated to "king" safety)
  ...
  // castling out of, through, and into check is allowed
