...
class AtomicVariantTest extends ChessTest:
  ...
  test("Must explode surrounding non pawn pieces on capture"):
    ...
    game
      ...
      .assertRight: game =>
        assert(explodedSquares.forall(square => game.position.pieceAt(square).isEmpty))
  ...
  test("The game must end by a traditional checkmate (atomic mate)"):
    ...
    game
      ...
      .assertRight: g =>
        assert(g.position.end)
        assertNot(g.position.variantEnd)
        assertEquals(g.position.winner, Some(Black))
  ...
  test(
    ...
    assertEquals(game.position.winner, None)
    assertEquals(game.position.moves.keySet, Set(Square.D4, Square.H7, Square.C8))
    game.position.moves
      .get(Square.D4)
      .assertSome: moves =>
  ...
  test(
    ...
    game
      ...
      .assertRight: game =>
        assertNot(game.position.end)
        assertEquals(game.position.winner, None)
        assertEquals(game.position.moves.keySet, Set(Square.D4, Square.H7))
        assert(game.position.legalMoves.forall(_.captures))
  ...
  test(
    ...
    game
      ...
      .assertRight: game =>
        assert(game.position.pieceAt(Square.D7).isDefined)
        assertNot(game.position.check.yes)
  ...
  test(
    ...
    game
      ...
      .assertRight: game =>
        assertEquals(game.position.pieceAt(Square.E6), None)
        ...
        assert:
          Square.E6.kingAttacks.forall: square =>
            game.position.pieceAt(square).isEmpty || square == Square.E7 || square == Square.D7
  ...
  test("After kings have been touching, and one moves away, a king that was protected is under attack again"):
    ...
    game
      ...
      .assertRight: game =>
        assertEquals(game.position.check, Check.Yes)
  ...
  test(
    ...
    game
      ...
      .assertRight: game =>
        assertEquals(game.position.check, Check.No)
  ...
  test("Allow castling with touching kings and rook shielding final attack"):
    ...
    fenToGame(position, Atomic)
      ...
      .assertRight: game =>
        assertEquals(game.position.pieceAt(Square.C1), White.king.some)
        assertEquals(game.position.pieceAt(Square.D1), White.rook.some)
  ...
  test("Allow castling with touching kings and rook shielding final attack 2"):
    ...
    fenToGame(position, Atomic)
      ...
      .assertRight: game =>
        assertEquals(game.position.pieceAt(Square.C8), Black.king.some)
        assertEquals(game.position.pieceAt(Square.D8), Black.rook.some)
  ...
  test("Exploded rooks can't castle"):
    ...
    fenToGame(position, Atomic)
      ...
      .assertRight: game =>
        assert(game.position.legalMoves.filter(_.castles).isEmpty)
