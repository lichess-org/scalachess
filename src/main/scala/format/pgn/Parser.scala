...
object Parser:
  ...
  private object MoveParser:
    ...
    val disambiguatedPawn: P[Std] = (((file.? ~ rank.?) ~ x).with1 ~ dest <* optionalEnPassant).map:
      case (((fi, ra), ca), de) =>
        Std(dest = de, role = Pawn, capture = ca, file = fi.flatMap(f => File(f - 1)), rank = ra.flatMap(r => Rank.atIndex(r - 1)))
    ...
    val disambiguated: P[Std] = (role ~ file.? ~ rank.? ~ x ~ dest).map:
      case ((((ro, fi), ra), ca), de) =>
        Std(dest = de, role = ro, capture = ca, file = fi.flatMap(f => File(f - 1)), rank = ra.flatMap(r => Rank.atIndex(r - 1)))
