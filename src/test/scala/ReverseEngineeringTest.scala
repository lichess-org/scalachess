...
*/
  ...
  "reverse engineer a move" should {
    "none on same games" in {
      "initial game" in {
        findMove(makeGame, makeGame) must beFailure
      }
      "played game" in {
    ...
    "none on different games" in {
      "initial to played" in {
        findMove(makeGame, playedGame) must beFailure
      }
      "played to initial" in {
        findMove(playedGame, makeGame) must beFailure
      }
    }
    "find one move" in {
      "initial game pawn moves one square" in {
        findMove(makeGame, play(makeGame, D2 -> D3)) must_== Success(D2 -> D3)
      }
      "initial game pawn moves two squares" in {
        findMove(makeGame, play(makeGame, D2 -> D4)) must_== Success(D2 -> D4)
      }
      "initial game bishop moves" in {
        findMove(makeGame, play(makeGame, B1 -> C3)) must_== Success(B1 -> C3)
      }
