...
class ForsythTest extends ChessTest {
  ...
  "the forsyth notation" should {
    "export" in {
      "game opening" in {
        ...
        "5 moves" in {
          makeGame.playMoveList(moves take 5) must beValid.like { case g =>
      ...
      "completely legal en-passant" in {
        makeGame.playMoves(A2 -> A4, G8 -> F6, A4 -> A5, B7 -> B5) must beValid.like { case g =>
      ...
      "standard castling rights" in {
        ...
        "inner rook" in {
          makeGame.playMoveList(moves dropRight 2) must beValid.like { case g =>
            f >> g must_== "rrbqkbn1/p1ppppp1/n7/1p5p/1P5P/N7/P1PPPPP1/RRBQKBN1 w Qq - 6 7"
          }
        }

        "inner rook removed" in {
          makeGame.playMoveList(moves) must beValid.like { case g =>
  ...
  "three-check" should {
    ...
    "write" in {
      "no checks" in {
        val moves = List(E2 -> E4, C7 -> C5, G1 -> F3, G8 -> H6, A2 -> A3)
        Game(ThreeCheck).playMoveList(moves take 5) must beValid.like { case g =>
      ...
      "checks" in {
        ...
        Game(ThreeCheck).playMoveList(moves) must beValid.like { case g =>
