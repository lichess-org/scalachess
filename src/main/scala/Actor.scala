...
final case class Actor(
      ...
      case Pawn =>
        pawnDir(pos) map { next =>
          ...
          def enpassant(horizontal: Direction): Option[Move] =
            for {
              victimPos <- horizontal(pos).filter(_ => pos.y0 == color.passablePawnY0)
          ...
          def maybePromote(m: Move): Option[Move] =
            if (m.dest.y0 == m.color.promotablePawnY0)
  ...
  def castleOn(side: Side): List[Move] =
    (for {
      ...
      newKingPos <- posAt0(side.castledKingX0, kingPos.y0)
      newRookPos <- posAt0(side.castledRookX0, rookPos.y0)
