...
class ParserTest extends ChessTest {
  ...
  "glyphs" in {
    parseMove("e4") must beValid.like {
    ...
    parseMove("Ne7g6+?!") must beValid.like {
      case a: Std =>
        a.dest === Pos.G6
        a.role === Knight
    ...
    parseMove("P@e4?!") must beValid.like {
      case a: Drop =>
        a.pos === Pos.E4
        a.role === Pawn
        a.metas.glyphs === Glyphs(Glyph.MoveAssessment.dubious.some, None, Nil)
  ...
  "comments" in {
    parser("Ne7g6+! {such a neat comment}") must beValid.like {
      case ParsedPgn(_, _, Sans(List(san))) =>
  ...
  "first move variation" in {
    parser("1. e4 (1. d4)") must beValid.like {
      case ParsedPgn(_, _, Sans(List(san))) =>
        san.metas.variations.headOption must beSome.like {
  ...
  "disambiguated" in {
    parser(disambiguated) must beValid.like {
  ...
  "variations" in {
    parser(variations) must beValid.like {
      case a => a.sans.value.size must_== 20
    }
  }

  "inline tags" in {
    parser(inlineTags) must beValid.like {
      case a =>
  ...
  "tag with nested quotes" in {
    parser("""[Black "Schwarzenegger, Arnold \"The Terminator\""]""") must beValid.like {
      case a =>
        a.tags.value must contain { (tag: Tag) =>
  ...
  "tag with inner brackets" in {
    parser("""[Black "[=0040.34h5a4]"]""") must beValid.like {
      case a =>
        a.tags.value must contain { (tag: Tag) =>
  ...
  "game from wikipedia" in {
    parser(fromWikipedia) must beValid.like {
      case a => a.sans.value.size must_== 85
    }
  }

  "game from crafty" in {
    parser(fromCrafty) must beValid.like {
      case a => a.sans.value.size must_== 68
    }
  }

  "inline comments" in {
    parser(inlineComments) must beValid.like {
      case a => a.sans.value.size must_== 85
    }
  }

  "comments and variations" in {
    parser(commentsAndVariations) must beValid.like {
      case a => a.sans.value.size must_== 103
    }
  }

  "comments and lines by smartchess" in {
    parser(bySmartChess) must beValid.like {
      case a => a.sans.value.size must_== 65
    }
  }

  "complete 960" in {
    parser(complete960) must beValid.like {
      case a => a.sans.value.size must_== 42
    }
  }

  "TCEC" in {
    parser(fromTcec) must beValid.like {
      case a => a.sans.value.size must_== 142
    }
  }

  "TCEC with engine output" in {
    parser(fromTcecWithEngineOutput) must beValid.like {
      case a => a.sans.value.size must_== 165
    }
  }

  "chesskids iphone" in {
    parser(chesskids) must beValid.like {
      case a => a.sans.value.size must_== 135
    }
  }

  "handwritten" in {
    parser(handwritten) must beValid.like {
      case a => a.sans.value.size must_== 139
    }
  }

  "chess by post" in {
    parser(chessByPost) must beValid.like {
      case a => a.sans.value.size must_== 100
    }
  }

  "Android device" in {
    parser(android) must beValid.like {
      case a => a.sans.value.size must_== 69
    }
  }

  "weird dashes" in {
    parser(weirdDashes) must beValid.like {
      case a => a.sans.value.size must_== 74
    }
  }

  "lichobile" in {
    parser(lichobile) must beValid.like {
      case a => a.sans.value.size must_== 68
    }
  }

  "overflow" in {
    parser(overflow) must beValid.like {
      case a => a.sans.value.size must_== 67
    }
  }
  "overflow 2" in {
    parser(stackOverflow) must beValid.like {
      case a => a.sans.value.size must_== 8
    }
  }
  "overflow 3" in {
    parser(overflow3) must beValid.like {
      case a => a.sans.value.size must_== 343
    }
  }
  "overflow 3: tags" in {
    Parser.TagParser.fromFullPgn(overflow3) must beValid.like {
      case tags => tags.value.size must_== 9
    }
  }
  "chessbase arrows" in {
    parser(chessbaseArrows) must beValid.like {
      case a =>
  ...
  "chessbase weird" in {
    parser(chessbaseWeird) must beValid.like {
      case a => a.sans.value.size must_== 115
    }
  }
  "crazyhouse from prod" in {
    parser(crazyhouseFromProd) must beValid.like {
      case a => a.sans.value.size must_== 49
    }
  }
  "crazyhouse from chess.com" in {
    parser(chessComCrazyhouse) must beValid.like {
      case a => a.sans.value.size must_== 42
    }
  }
  "en passant e.p. notation" in {
    parser(enpassantEP) must beValid.like {
      case a => a.sans.value.size must_== 36
    }
    parser(enpassantEP2) must beValid.like {
      case a => a.sans.value.size must_== 36
    }
  }
