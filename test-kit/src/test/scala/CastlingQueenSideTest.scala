...
PPPPPPPP
R   KB R"""
  val badHist = goodHist.updateHistory(_.withoutCastlingRights(White))
  ...
  test("possible: viable moves"):
    ...
    assertEquals(game.position.destsFrom(E1), Set(A1, C1, D1))
...
R    RK """
    ...
    assertEquals(g2.position.destsFrom(G1), Set(H1))
    assertEquals(
      g2.position
        .seq(
          _.move(F1, H1),
...
PPPPPPPP
  KR   R"""
    ...
    assertEquals(g2.position.destsFrom(C1), Set(B1))
    assertEquals(
      g2.position
        .seq(
          _.move(D1, A1),
