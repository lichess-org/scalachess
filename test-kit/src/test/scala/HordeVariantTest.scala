...
class HordeVariantTest extends ChessTest:
  ...
  test("Must not be insufficient winning material for horde with only 1 pawn left"):
    ...
    assertNot(game.get.situation.opponentHasInsufficientMaterial)
  ...
  test("Must recognise insufficient winning material for horde with only 1 pawn left"):
    ...
    val game     = fenToGame(position, Horde)
    assert(game.get.situation.opponentHasInsufficientMaterial)
  ...
  test("Must recognise insufficient winning material for horde with only 1 bishop left"):
    ...
    val game     = fenToGame(position, Horde)
    assertNot(game.get.situation.autoDraw)
    assertNot(game.get.situation.end)
    assert(game.get.situation.opponentHasInsufficientMaterial)
  ...
  test("Must auto-draw in simple pawn fortress"):
    ...
    val game     = fenToGame(position, Horde)
    assert(game.get.situation.autoDraw)
    assert(game.get.situation.opponentHasInsufficientMaterial)
  ...
  test("Must not auto-draw if not all black King moves leads to stalemate"):
    ...
    assertNot(game.get.situation.autoDraw)
    assertNot(game.get.situation.end)
    assertEquals(game.get.situation.status, None)
    assertNot(Horde.isInsufficientMaterial(game.get.situation.board))
    assertNot(game.get.situation.opponentHasInsufficientMaterial)
  ...
  test("Pawn on first rank should able to move two squares"):
    ...
    val game     = fenToGame(position, Horde)
    assert(game.get.situation.legalMoves.exists(m => m.orig == Square.D1 && m.dest == Square.D3))

  test("Cannot en passant a pawn from the first rank"):
    val position = EpdFen("k7/5p2/4p2P/3p2P1/2p2P2/1p2P2P/p2P2P1/2P2P2 w - - 0 1")
    val game     = fenToGame(position, Horde).get.apply(Square.C1, Square.C3)
    assertNot(game.get._1.situation.legalMoves.exists(m => m.orig == Square.B3 && m.dest == Square.C2))
  ...
  test("UnmovedRooks & castles at the starting position"):
    ...
    assertEquals(board.history.castles, Castles("kq"))

  test("the h8 rooks move"):
    val position = EpdFen("r3kbnr/p1pqppp1/1pnp3P/PPPP1P1P/PPP1PPP1/1PPP1PPP/PPPPPPPP/PPPPPPPP b kq - 0 7")
    val game     = fenToGame(position, Horde).get.apply(Square.H8, Square.H6)
    assertEquals(game.get._1.situation.board.history.unmovedRooks, UnmovedRooks(Set(Square.A8)))
    assertEquals(game.get._1.situation.board.history.castles, Castles("q"))
