...
class ForsythTest extends ChessTest:
  ...
  "dump" should {
    "enpassant" in {
      ...
      Forsyth.>>(sit.state) must_== FEN("rnbqkbnr/pp1p1ppp/8/2pPp3/8/8/PPP1PPPP/RNBQKBNR w KQkq c6 0 3")
      sit.state.situation.destinations.get(D5) must_== Some(List(D6, C6))
    }
    "enpassant in game" in {
      ...
      sit.state.situation.destinations.get(E4) must_== Some(List(E3, D3))
    }
    "not enpassant in game" in {
      ...
      val sit = Reader.full(pgn).flatMap(_.valid).toOption.get
      sit.state.situation.destinations.get(G5) must_== None
      Forsyth.>>(sit.state) must_== FEN("3r1r2/1p5k/pb1q2p1/3p2Pp/4pP2/2B1P1P1/PP1Q2K1/R6R w - - 0 28")
    }
  }
